...
module.exports = function (server) {
    ...
    this.server = server;
    var sockJSURL = config("site.protocol", "http") + "://" + config("site.domain", "localhost:8080") + "/" + config("site.pathPrefix", "") + "js/3rdparty/sockjs-1.1.0.min.js";
    ...
    this.wsserver.on("connection", function (conn) {
        ...
        if (ddosProtection) {
            ...
            if (count > connectionLimit) {
                Global.error("DDoS detected (WebSocket/connection):", Tools.preferIPv4(conn.ip), count, connectionLimit);
                return conn.end();
            }
        ...
        conn.on("data", function (message) {
            ...
            if (ddosProtection && message.length > maxMessageLength) {
                Global.error("DDoS detected (WebSocket/message):", Tools.preferIPv4(conn.ip), message.length, maxMessageLength);
                return conn.end();
            }
            try {
            ...
            switch (message.type) {
                case "init":
                    if (connectionsIP.hasOwnProperty(conn.ip)) connectionsIP[conn.ip].push(conn);else connectionsIP[conn.ip] = [conn];
                    if (message.data && message.data.hashpass) {
                        Object.defineProperty(conn, "hashpass", { value: message.data.hashpass });
                        if (connectionsHashpass.hasOwnProperty(conn.hashpass)) connectionsHashpass[conn.hashpass].push(conn);else connectionsHashpass[conn.hashpass] = [conn];
                    }
                ...
                default:
                    ...
                    var replyData = function replyData(data) {
                    ...
                    var replyErr = function replyErr(err) {
                        conn.write(JSON.stringify({
                            id: message.id,
                            type: message.type,
                            error: error
        ...
        conn.on("close", function () {
            if (ddosProtection) --connectionCount[conn.ip];
            if (connectionsIP.hasOwnProperty(conn.ip)) {
                var list = connectionsIP[conn.ip];
                list.some(function (c, i) {
                    if (c == conn) list.splice(i, 1);
                    if (list.length < 1) delete connectionsIP[conn.ip];
                });
            }
            if (conn.hashpass && connectionsHashpass.hasOwnProperty(conn.hashpass)) {
                ...
                list.some(function (c, i) {
                    if (c == conn) list.splice(i, 1);
                    if (list.length < 1) delete connectionsHashpass[conn.ip];
                });
            }
        });
    });
...
module.exports.prototype.sendMessage = function (type, data, ips, hashpasses) {
    if (!type) return;
    if (!Util.isArray(ips)) ips = [ips];
    if (!Util.isArray(hashpasses)) hashpasses = [hashpasses];
