...
var checkPermissions = function checkPermissions(req, board, post, permission, password) {
    ...
    if (compareRegisteredUserLevels(req.level(board.name), Permissions[permission]()) >= 0) {
        if (compareRegisteredUserLevels(req.level(board.name), post.user.level) > 0) return Promise.resolve(true);
...
var getThreads = function getThreads(boardName, options) {
    if (!Tools.contains(Board.boardNames(), boardName)) return Promise.reject(Tools.translate("Invalid board"));
    var opts = (typeof options === "undefined" ? "undefined" : _typeof(options)) == "object";
    var c = {};
    ...
    var p = db.hgetall(key).then(function (result) {
        ...
        for (var number in result) {
            if (!result.hasOwnProperty(number)) continue;
            var thread = JSON.parse(result[number]);
            if (!filter || options.filterFunction(thread)) threads.push(thread);
    ...
    if (!opts || !options.withPostNumbers) return p;
...
module.exports.registerUser = function (password, levels, ips, notHashpass) {
    if (!password) return Promise.reject(Tools.translate("Invalid password"));
    var hashpass = toHashpass(password, notHashpass);
    if (!Tools.hasOwnProperties(levels)) return Promise.reject(Tools.translate("Access level is not specified for any board"));
    var invalidBoard = Object.keys(levels).some(function (boardName) {
        if (!Board.board(boardName)) {
            return true;
        }
    });
    if (invalidBoard) return Promise.reject(Tools.translate("Invalid board"));
    var invalidLevel = Tools.toArray(levels).some(function (level) {
        if (compareRegisteredUserLevels(level, RegisteredUserLevels.User) < 0 || compareRegisteredUserLevels(level, RegisteredUserLevels.Superuser) >= 0) {
            return true;
        }
    });
    if (invalidLevel) return Promise.reject(Tools.translate("Invalid access level"));
    var invalidIp;
    if (Util.isArray(ips)) {
        invalidIp = ips.some(function (ip, i) {
            ip = Tools.correctAddress(ip);
    ...
    return db.exists("registeredUserLevels:" + hashpass).then(function (result) {
        ...
        if (result) return Promise.reject(Tools.translate("A user with this hashpass is already registered as superuser"));
        return db.hmset("registeredUserLevels:" + hashpass, levels);
    }).then(function () {
        if (!Util.isArray(ips)) return Promise.resolve();
        return Tools.series(ips, function (ip) {
            return db.hset("registeredUserHashes", ip, hashpass).then(function () {
                return db.sadd("registeredUserIps:" + hashpass, ip);
            });
...
module.exports.unregisterUser = function (hashpass) {
    if (!hashpass || !Tools.mayBeHashpass(hashpass)) return Promise.reject(Tools.translate("Invalid hashpass"));
...
module.exports.updateRegisteredUser = function (hashpass, levels, ips, notHashpass) {
    ...
    if (!Tools.hasOwnProperties(levels)) return Promise.reject(Tools.translate("Access level is not specified for any board"));
    var invalidBoard = Object.keys(levels).some(function (boardName) {
        if (!Board.board(boardName)) return true;
    });
    if (invalidBoard) return Promise.reject(Tools.translate("Invalid board"));
    var invalidLevel = Tools.toArray(levels).some(function (level) {
        if (compareRegisteredUserLevels(level, RegisteredUserLevels.User) < 0 || compareRegisteredUserLevels(level, RegisteredUserLevels.Superuser) >= 0) {
...
var registeredUserLevel = function registeredUserLevel(password, boardName, notHashpass) {
    if (!password) return Promise.reject(Tools.translate("Invalid password"));
    if (!Board.board(boardName)) return Promise.reject(Tools.translate("Invalid board"));
    var hashpass = toHashpass(password, notHashpass);
    return db.sismember("superuserHashes", hashpass).then(function (result) {
        if (!result) return db.hgetall("registeredUserLevels:" + hashpass);
...
var registeredUserLevelByIp = function registeredUserLevelByIp(ip, boardName) {
    ip = Tools.correctAddress(ip);
    ...
    return db.hget("registeredUserHashes", ip).then(function (hashpass) {
        if (!hashpass) return Promise.resolve(null);
...
var registeredUserLevels = function registeredUserLevels(reqOrHashpass) {
    if (reqOrHashpass && (typeof reqOrHashpass === "undefined" ? "undefined" : _typeof(reqOrHashpass)) == "object" && reqOrHashpass.cookies) reqOrHashpass = Tools.hashpass(reqOrHashpass);
    if (!reqOrHashpass) return Promise.resolve(null);
    return db.sismember("superuserHashes", reqOrHashpass).then(function (result) {
        if (!result) return db.hgetall("registeredUserLevels:" + reqOrHashpass);
...
var registeredUserLevelsByIp = function registeredUserLevelsByIp(ip) {
    ip = Tools.correctAddress(ip);
    if (!ip) return Promise.reject(Tools.translate("Invalid IP address"));
    return db.hget("registeredUserHashes", ip).then(function (hashpass) {
        if (!hashpass) return Promise.resolve({});
...
var nextPostNumber = function nextPostNumber(boardName, incrby) {
    var board = Board.board(boardName);
    if (!board) return Promise.reject(Tools.translate("Invalid board"));
    incrby = +incrby;
    if (isNaN(incrby) || incrby < 1) incrby = 1;
    return db.hincrby("postCounters", boardName, incrby).then(function (number) {
        if (!number) return 0;
        if (1 == incrby && board.skippedGetOrder > 0 && !(number % Math.pow(10, board.skippedGetOrder))) return nextPostNumber(boardName, incrby);
        return number;
    });
};
...
module.exports.getFileInfosByHashes = function (hashes) {
    if (!hashes) return Promise.resolve([]);
    if (!Util.isArray(hashes)) hashes = [hashes];
...
var createPost = function createPost(req, fields, files, transaction, threadNumber, date) {
    var board = Board.board(fields.boardName);
    if (!board) return Promise.reject(Tools.translate("Invalid board"));
    if (!board.postingEnabled) return Promise.reject(Tools.translate("Posting is disabled at this board"));
    date = date || Tools.now();
    var c = {};
    if (threadNumber) c.postNumber = threadNumber;else threadNumber = +fields.threadNumber;
    ...
    Tools.forIn(markup.MarkupModes, function (val) {
        if (fields.markupMode && fields.markupMode.indexOf(val) >= 0) markupModes.push(val);
    });
    return getThreads(board.name, {
        ...
        if (!threads || threads.length != 1) return Promise.reject(Tools.translate("No such thread"));
        if (threads[0].closed) return Promise.reject(Tools.translate("Posting is disabled in this thread"));
        ...
        if (c.postCount >= board.postLimit) return Promise.reject(Tools.translate("Post limit reached"));
        ...
        c.geo = geo;
        if (c.postNumber) return Promise.resolve(c.postNumber);else return nextPostNumber(board.name);
        ...
        if (c.postCount >= board.bumpLimit) return Promise.resolve();
        if (c.unbumpable) return Promise.resolve();
...
var checkCaptcha = function checkCaptcha(req, fields) {
    var board = Board.board(fields.boardName);
    if (!board) return Promise.reject(Tools.translate("Invalid board"));
    if (!board.captchaEnabled) return Promise.resolve();
    var ip = req.ip;
    return UsersModel.getUserCaptchaQuota(board.name, ip).then(function (quota) {
        if (board.captchaQuota > 0 && +quota > 0) return captchaUsed(board.name, ip);
        var supportedCaptchaEngines = board.supportedCaptchaEngines;
        if (supportedCaptchaEngines.length < 1) return Promise.reject(Tools.translate("Internal error"));
        var ceid = fields.captchaEngine;
        var isSupported = function isSupported(id) {
            for (var i = 0; i < supportedCaptchaEngines.length; ++i) {
                if (id === supportedCaptchaEngines[i]) return true;
            }
            return false;
        };
        if (!ceid || !isSupported(ceid)) {
            if (isSupported("node-captcha")) ceid = "node-captcha";else ceid = supportedCaptchaEngines[0];
        }
        var captcha = Captcha.captcha(ceid);
        if (!captcha) return Promise.reject(Tools.translate("Invalid captcha engine"));
...
module.exports.createPost = function (req, fields, files, transaction) {
    ...
    var c = {};
    if (isNaN(threadNumber) || threadNumber <= 0) return Promise.reject(Tools.translate("Invalid thread"));
...
module.exports.createThread = function (req, fields, files, transaction) {
    ...
    return checkCaptcha(req, fields).then(function () {
        ...
        if (c.threads.length < board.threadLimit) return Promise.resolve();
        return getThreads(board.name, { archived: true }).then(function (threads) {
            ...
            if (c.archivedThreads.length <= 0 || c.archivedThreads.length < board.archiveLimit) return Promise.resolve();
            return removeThread(board.name, c.archivedThreads.pop().number, { archived: true });
        }).then(function () {
            c.thread = c.threads.pop();
            if (board.archiveLimit <= 0) return removeThread(board.name, c.thread.number);
...
var compareRegisteredUserLevels = function compareRegisteredUserLevels(l1, l2) {
...
var getGeolocationInfo = function getGeolocationInfo(ip) {
    var info = {
        cityName: null,
        countryCode: null,
        countryName: null
    };
    if (!ip) return Promise.resolve(info);
    var address = Tools.correctAddress(ip);
    if (!address) return Promise.resolve(info);
    ...
    if (ipv4) address = bigInt(new Address4(ipv4).bigInteger().toString());else address = bigInt(new Address6(address).bigInteger().toString());
    return stmt.pget(address.toString()).then(function (result) {
        ...
        if (ipFrom.greater(address)) return info;
...
module.exports.addFiles = function (req, fields, files, transaction) {
    ...
    if (isNaN(postNumber) || postNumber <= 0) return Promise.reject(Tools.translate("Invalid post number"));
    return PostsModel.getPost(board.name, postNumber, { withExtraData: true }).then(function (post) {
        ...
        if (!result) return Promise.reject(Tools.translate("Not enough rights"));
        return postFileInfoNames(board.name, c.post.number);
    }).then(function (names) {
        if (names.length + files.length > board.maxFileCount) return Promise.reject(Tools.translate("Too many files"));
        return processFiles(req, fields, files, transaction);
    }).then(function (files) {
        c.files = files;
...
module.exports.editPost = function (req, fields) {
    ...
    return PostsModel.getPost(board.name, postNumber, { withExtraData: true }).then(function (post) {
        if (!post) return Promise.reject(Tools.translate("Invalid post"));
        c.post = post;
        ...
        if (!rawText && numbers.length < 1) return Promise.reject(Tools.translate("Both file and comment are missing"));
        if (rawText && rawText.length > board.maxTextLength) return Promise.reject(Tools.translate("Text is too long"));
        if (name && name.length > board.maxNameLength) return Promise.reject(Tools.translate("Name is too long"));
        if (subject && subject.length > board.maxSubjectLength) return Promise.reject(Tools.translate("Subject is too long"));
        return markup(board.name, rawText, {
            markupModes: markupModes,
            referencedPosts: referencedPosts,
...
module.exports.deletePost = function (req, res, fields) {
    ...
    return UsersModel.checkUserBan(req.ip, board.name, { write: true }).then(function () {
        ...
        if (c.archived && !c.isThread) return Promise.reject(Tools.translate("Deleting posts from archived threads is not allowed"));
        return checkPermissions(req, board, post, "deletePost", Tools.sha1(fields.password));
    }).then(function (result) {
        if (!result) return Promise.reject(Tools.translate("Not enough rights"));
        return c.isThread ? removeThread(board.name, postNumber, { archived: c.archived }) : removePost(board.name, postNumber);
    }).then(function () {
        var p;
        if (c.isThread && c.archived) {
            ...
            p = c.isThread ? IPC.render(c.post.boardName, c.post.threadNumber, c.post.number, 'delete') : IPC.render(c.post.boardName, c.post.threadNumber, c.post.number, 'edit');
        }
        return p;
    }).then(function () {
        return {
...
module.exports.moveThread = function (req, fields) {
    ...
    if (!sourceBoard || !targetBoard) return Promise.reject(Tools.translate("Invalid board"));
    if (sourceBoard.name == targetBoard.name) return Promise.reject(Tools.translate("Source and target boards are the same"));
    var threadNumber = +fields.threadNumber;
    if (isNaN(threadNumber) || threadNumber <= 0) return Promise.reject(Tools.translate("Invalid thread number"));
    if (!req.isModer(sourceBoard.name) || !req.isModer(targetBoard.name)) return Promise.reject(Tools.translate("Not enough rights"));
    var password = Tools.sha1(fields.password);
    var c = {};
    ...
    return getThreads(sourceBoard.name, {
        ...
        if (!thread || thread.length != 1) return Promise.reject(Tools.translate("No such thread"));
        c.thread = thread[0];
        if ((!password || password != c.thread.user.password) && (!req.hashpass || req.hashpass != c.thread.user.hashpass) && !req.isSuperuser() && compareRegisteredUserLevels(req.level(sourceBoard.name), c.thread.user.level) <= 0) {
        ...
        return Tools.series(c.posts, function (post) {
            ...
            if (post.rawText) {
                ...
                referencedPosts.forEach(function (ref) {
                    if (ref.boardName != sourceBoard.name) return;
            ...
            return db.hset("posts", targetBoard.name + ":" + post.number, JSON.stringify(post)).then(function () {
                ...
                return Tools.series(referencedPosts, function (ref) {
                    ...
                    return db.hdel("referencedPosts:" + sourceBoard.name + ":" + oldPostNumber, ref.boardName + ":" + ref.postNumber).then(function () {
                        return db.hset("referencedPosts:" + targetBoard.name + ":" + post.number, nref.boardName + ":" + nref.postNumber, JSON.stringify(nref));
                    });
                });
            }).then(function () {
                return Tools.series(referringPosts, function (ref) {
                    ...
                    return db.hdel("referringPosts:" + sourceBoard.name + ":" + oldPostNumber, ref.boardName + ":" + ref.postNumber).then(function () {
                        return db.hset("referringPosts:" + targetBoard.name + ":" + post.number, nref.boardName + ":" + nref.postNumber, JSON.stringify(nref));
                    });
                });
            }).then(function () {
        ...
        return Tools.series(c.toRerender, function (o) {
            return PostsModel.getPost(o.boardName, o.postNumber).then(function (p) {
                if (!p.rawText) return Promise.resolve();
                Tools.forIn(c.postNumberMap, function (newPostNumber, previousPostNumber) {
                    var rx = new RegExp(">>/" + sourceBoard.name + "/" + previousPostNumber, "g");
...
module.exports.deleteFile = function (req, res, fields) {
    ...
    if (!fileName) return Promise.reject(Tools.translate("Invalid file name"));
    var c = {};
    return db.hget("fileInfos", fileName).then(function (fileInfo) {
        ...
        if (!c.post.rawText && numbers.length < 2) return Promise.reject(Tools.translate("Both file and comment are missing"));
...
module.exports.editFileRating = function (req, res, fields) {
    ...
    return getFileInfo({ fileName: fields.fileName }).then(function (fileInfo) {
        ...
        if (!post) return Promise.reject(Tools.translate("Invalid post"));
        c.post = post;
        if ((!password || password != post.user.password) && (!req.hashpass || req.hashpass != post.user.hashpass) && !req.isSuperuser() && compareRegisteredUserLevels(req.level(c.fileInfo.boardName), post.user.level) <= 0) {
            return Promise.reject(Tools.translate("Not enough rights"));
        }
        c.fileInfo.rating = "SFW";
        if (["R-15", "R-18", "R-18G"].indexOf(fields.rating) >= 0) c.fileInfo.rating = fields.rating;
...
module.exports.editAudioTags = function (req, res, fields) {
    ...
    var password = Tools.sha1(fields.password);
    return getFileInfo({ fileName: fields.fileName }).then(function (fileInfo) {
        if (!Tools.isAudioType(fileInfo.mimeType)) return Promise.reject(Tools.translate("Not an audio file"));
        c.fileInfo = fileInfo;
        return UsersModel.checkUserBan(req.ip, c.fileInfo.boardName, { write: true });
    }).then(function () {
        return PostsModel.getPost(c.fileInfo.boardName, c.fileInfo.postNumber);
...
var updatePostBanInfo = function updatePostBanInfo(boardName, postNumber) {
    if (!Board.board(boardName)) return Promise.reject(Tools.translate("Invalid board"));
    postNumber = +postNumber;
    if (isNaN(postNumber) || postNumber <= 0) return Promise.resolve();
...
var updateBan = function updateBan(ip, boardName, postNumber) {
    ip = Tools.correctAddress(ip);
    if (!ip) return Promise.reject(Tools.translate("Invalid IP address"));
    if (!Board.board(boardName)) return Promise.reject(Tools.translate("Invalid board"));
...
module.exports.banUser = function (req, ip, bans) {
    ...
    if (!req.isModer()) return Promise.reject(Tools.translate("Not enough rights"));
    ...
    return UsersModel.getBannedUserBans(ip).then(function (oldBans) {
        ...
        newBans = Board.boardNames().reduce(function (acc, boardName) {
            if (req.isModer(boardName)) {
                ...
                if (oldBans.hasOwnProperty(boardName)) acc[boardName] = oldBans[boardName];
            }
            return acc;
        }, {});
        ...
        return Tools.series(Board.boardNames(), function (boardName) {
            ...
            if (!ban) {
                ...
                if (!ban) return Promise.resolve();
                return db.del(key).then(function () {
                    if (!ban.postNumber) return Promise.resolve();
            ...
            return db.set(key, JSON.stringify(ban)).then(function () {
                if (!ban.expiresAt) return Promise.resolve();
...
module.exports.delall = function (req, ip, boardNames) {
    ...
    return registeredUserLevelsByIp(ip).then(function (levels) {
        err = boardNames.some(function (boardName) {
            var level = req.level(boardName);
            if (!req.isSuperuser(boardName) && compareRegisteredUserLevels(level, levels[boardName]) <= 0) return true;
        });
        if (err) return Promise.reject(Tools.translate("Not enough rights"));
...
module.exports.initialize = function () {
    ...
    var dbs = (0, _clientFactory2.default)(true);
    var initialized = false;
    var query = [];
    var updateBanOnMessage = function updateBanOnMessage(message) {
