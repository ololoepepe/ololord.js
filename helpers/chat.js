...
var createHash = function createHash(user) {
...
module.exports.sendMessage = function (user, boardName, postNumber, text) {
    if (!text) return Promise.reject(Tools.translate("Message is empty"));
    if (!boardName) return Promise.reject(Tools.translate("Invalid board"));
    postNumber = +postNumber;
    if (!boardName || !postNumber || postNumber < 0) return Promise.reject(Tools.translate("Invalid post number"));
    var c = {};
    ...
    return PostsModel.getPost(boardName, postNumber).then(function (post) {
        ...
        if (msg && msg.length > 0 && JSON.parse(msg[0]).senderHash != c.senderHash && JSON.parse(msg[0]).receiverHash != c.senderHash) {
        ...
        if (c.senderHash == c.receiverHash) {
            c.members.some(function (member) {
                ...
                if (member.hash == c.senderHash) return;
...
module.exports.getMessages = function (user, lastRequestDate) {
    ...
    if (!lastRequestDate) lastRequestDate = 0;
    ...
    return Database.db.smembers("chats:" + hash).then(function (keys) {
        return Tools.series(keys, function (key) {
            return Database.db.zrangebyscore("chat:" + key, lastRequestDate, Infinity).then(function (list) {
                ...
                if (list.length > 0) chats[key] = list;
            });
        });
    }).then(function () {
        return Promise.resolve({
